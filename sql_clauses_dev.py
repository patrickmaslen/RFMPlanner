...
def calculate_interim_fmas_combined_no_prioritisn():
    ...
    INSERT INTO table_name (fma_type, name, source, geometry)
    SELECT 
        result.fma_type,
        result.name,
        result.source,
    ...
    GROUP BY fma_type, next_in.name, done.geometry
    ...
    SELECT fma as fma_type, name, 'New' AS source, geometry FROM input1
    ) AS result;
    DELETE FROM table_name WHERE ST_IsEmpty(geometry);
    """    

def calculate_interim_fmas_combined_no_prioritisnOLD():
    # called by rfm_library.create_interim
    return """
    DROP TABLE IF EXISTS table_name;
    CREATE TABLE public.table_name
    ...
    FROM input2 done, input1 next_in
    GROUP BY fma_type, done.geometry, next_in.geometry
    HAVING NOT ST_Intersects(done.geometry, next_in.geometry)
...
def calculate_intersects():
    ...
    GROUP BY name, priority, be.geometry, buff.geometry;
...
def create_fmas_complete():
    ...
    SELECT fma_type, name, priority, ST_Multi(ST_CollectionExtract((ST_Dump(ST_MakeValid(ST_Union(ST_Buffer(geometry, 0))))).geom, 3))::geometry(MultiPolygon,900914) AS geometry
    FROM region_fmas_combined 
    GROUP BY fma_type, name, priority;
...
def create_region_fuel_type_age_with_thold_code():
    ...
    CREATE TABLE public.region_fuel_type_age_with_thold_code
    (
        fuel_type character varying COLLATE pg_catalog."default",
        threshold_age integer,
        yslb integer,
    ...
    ALTER TABLE public.fuel_type_age_with_thold_cib
        OWNER to rfmp;
...
def fill_region_fuel_type_age_with_thold_code_OLD():
    return """
    INSERT INTO region_fuel_type_age_with_thold_code(fuel_type, threshold_age, yslb, diff, target, geometry)
    SELECT f.fuel_type fuel_type, t.threshold_age threshold_age, f.yslb yslb, yslb - threshold_age diff, t.target target, f.geometry geometry
    FROM region_fuel_type_age_with_thold f, region_default_thresholds t WHERE f.code = True and t.fma_type = 'CODE' and f.fuel_type = t.fuel_type;
...
def regional_areas_by_fma_OLD():    # Delete if Pilbara works OK on a repeat run
    return """select zone, fma_type,
    ST_Area(ST_Collect(case when ST_CoveredBy(p.geometry, z.geometry) then p.geometry
            else ST_MakeValid(PolygonalIntersection(p.geometry, z.geometry))
            END))/10000 AS Area_ha 
     FROM region_fmas_combined AS p 
       INNER JOIN brmzs_region AS z
        ON ST_Intersects(p.geometry,z.geometry)
    group by zone, fma_type;
    """
...
def regional_areas_past_indic_thold_OLD():    # Delete if Pilbara works OK on a repeat run
    return """select zone, fma_type, target, reached_th,
    ST_Area(ST_Collect(case when ST_CoveredBy(p.geometry, z.geometry) then p.geometry
            else PolygonalIntersection(p.geometry, z.geometry)
            END))/10000 AS Area_ha 
     FROM region_past_indic_thold AS p 
       INNER JOIN brmzs_region AS z
        ON ST_Intersects(p.geometry,z.geometry)
    group by zone, fma_type, target, reached_th;
    """
...
def threshold_calculation_OLD():
    ...
    UPDATE region_with_indic_thold r SET target = (SELECT target FROM fuel_type_indicative_thresholds f WHERE f.fuel_type = r.fuel_type AND f.fma_type = r.fma_type);
    UPDATE region_with_indic_thold r SET target = -1 WHERE fuel_type = 'Non-fuel';
    """

def threshold_calculation_definitive_OLD():
    from rfm_library import debug_msg_box as debug
    debug("threshold_calculation_definitive")
    # called by rfm_library.calculate_defin_tholds
    return """
    INSERT INTO region_with_thold(fma_type, fuel_type, threshold_age, yslb, geometry)
    SELECT xyz.fma_type, xyz.fuel_type, xyz.threshold_age, xyz.yslb, xyz.geometry::geometry(Polygon,900914) AS geometry
    FROM (SELECT rfc.fma_type, t.fuel_type, t.threshold_age, t.yslb, (ST_Dump(PolygonalIntersection(ST_Buffer(rfc.geometry, 0.001), ST_Buffer(t.geometry, 0.001)))).geom AS geometry
        FROM region_fmas_complete rfc, region_fuel_type_age_with_thold_code t WHERE rfc.fma_type = 'CODE' AND ST_Intersects(rfc.geometry, t.geometry) AND NOT  st_touches(rfc.geometry, t.geometry)) AS xyz;
    ...
    UPDATE region_with_thold r SET target = (SELECT shs_target FROM region_default_thresholds f WHERE f.fuel_type = r.fuel_type AND r.fma_type = 'SHS');
	UPDATE region_with_thold r SET target = (SELECT cib_target FROM region_default_thresholds f WHERE f.fuel_type = r.fuel_type AND r.fma_type = 'CIB');
	UPDATE region_with_thold r SET target = (SELECT lrr_target FROM region_default_thresholds f WHERE f.fuel_type = r.fuel_type AND r.fma_type = 'LRR');
    UPDATE region_with_thold r SET target = -1 WHERE fuel_type = 'Non-fuel';
    """

def threshold_calculation_definitive_VERY_OLD():
    # called by rfm_library.calculate_defin_tholds
    return """
    INSERT INTO region_with_thold(fma_type, fuel_type, threshold_age, yslb, geometry)
    SELECT xyz.fma_type, xyz.fuel_type, xyz.threshold_age, xyz.yslb, xyz.geometry::geometry(Polygon,900914) AS geometry
    ...
    UPDATE region_with_thold r SET target = (SELECT target FROM fuel_type_indicative_thresholds f WHERE f.fuel_type = r.fuel_type AND f.fma_type = r.fma_type);
    UPDATE region_with_thold r SET target = -1 WHERE fuel_type = 'Non-fuel';
    """
    
def threshold_calculationOLD():
    return """
    INSERT INTO region_with_indic_thold(fma_type, fuel_type, threshold_age, yslb, geometry)
    SELECT xyz.fma_type, xyz.fuel_type, xyz.threshold_age, xyz.yslb, xyz.geometry::geometry(Polygon,900914) AS geometry
    FROM (SELECT rfc.fma_type, t.fuel_type, t.threshold_age, t.yslb, (ST_Dump(PolygonalIntersection(ST_Buffer(rfc.geometry, 0.001), ST_Buffer(t.geometry, 0.001)))).geom AS geometry
