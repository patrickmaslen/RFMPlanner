...
def calculate_buffers(region_to_buffer, fma_type, fcs_to_buffer):
    ...
    try:
        ...
        if globals.prioritisation_on:
            create_sql = "CREATE TABLE " + temp_intersect_tbl_name + " (priority integer, geometry geometry(MultiPolygon, 900914))"
        else:
            create_sql = "CREATE TABLE " + temp_intersect_tbl_name + " (geometry geometry(MultiPolygon, 900914))"
        ...
        for buffer_distance in buffer_distances:
            if buffer_distance[0] is not None:
                ...
                if globals.prioritisation_on:
                    create_sql = "CREATE TABLE " + temp_buffer_tbl_name + " (priority integer, geometry geometry(MultiPolygon, 900914))"
                else:
                    create_sql = "CREATE TABLE " + temp_buffer_tbl_name + " (geometry geometry(MultiPolygon, 900914))"
...
def show_dept_land(detail_or_boundaries):
    layer = load_postgis_layer("dept_managed_land_" + detail_or_boundaries)
...
def create_tables(region, tables_to_create, default_tholds_exist, spatial_tholds_exist):
    # Called by check_threshold_table_dates - can prob be deleted
...
def style_layer(tbl_name, vlayer, fma_grouping):
    if os.path.isfile(os.path.join(rfm_resources_dir, tbl_name + ".qml")):
        ...
        if vlayer.name().endswith("_fuel_type") or "fuel_type_all" in vlayer.name():
